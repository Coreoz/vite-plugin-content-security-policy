// import {
//   computeDirectives,
//   generateNonce,
//   replaceNoncePlaceholder,
// } from '@lib/csp-proxy/CspProxy';
// import { CspPolicies } from '@lib/csp/CspDirectives';
// import { describe, expect, it } from 'vitest';
//
// type Environment = 'production' | 'staging';
//
// describe('generateNonce', () => {
//   it('should generate a random base64 string', () => {
//     const nonce: string = generateNonce();
//
//     // Check that the nonce is a non-empty string
//     expect(nonce).toBeDefined();
//     expect(typeof nonce).toBe('string');
//     expect(nonce.length).toBeGreaterThan(0);
//
//     // Check that it's a valid base64 string
//     const base64Regex: RegExp = /^[A-Za-z0-9+/=]+$/;
//     expect(base64Regex.test(nonce)).toBe(true);
//   });
//
//   it('should generate different nonces on each call', () => {
//     const nonce1: string = generateNonce();
//     const nonce2: string = generateNonce();
//
//     expect(nonce1).not.toBe(nonce2);
//   });
// });
//
// describe('replaceRandomPlaceholder', () => {
//   it('should replace nonce-{RANDOM} with the provided nonce', () => {
//     const value: string = '\'self\' \'unsafe-inline\' nonce-{RANDOM}';
//     const nonce: string = 'test-nonce-value';
//
//     const result: string = replaceNoncePlaceholder(value, nonce);
//
//     expect(result).toBe('\'self\' \'unsafe-inline\' nonce-test-nonce-value');
//   });
//
//   it('should not modify strings without the placeholder', () => {
//     const value: string = '\'self\' \'unsafe-inline\'';
//     const nonce: string = 'test-nonce-value';
//
//     const result: string = replaceNoncePlaceholder(value, nonce);
//
//     expect(result).toBe('\'self\' \'unsafe-inline\'');
//   });
//
//   it('should replace multiple occurrences of the placeholder', () => {
//     const value: string = 'nonce-{RANDOM} \'self\' nonce-{RANDOM}';
//     const nonce: string = 'test-nonce-value';
//
//     const result: string = replaceNoncePlaceholder(value, nonce);
//
//     expect(result).toBe('nonce-test-nonce-value \'self\' nonce-test-nonce-value');
//   });
// });
//
// describe('computeDevelopmentDirective', () => {
//   it('should compute development directive with string values', () => {
//     const policies: CspPolicies<Environment> = {
//       'default-src': '\'self\'',
//       'script-src': '\'self\' \'unsafe-inline\'',
//       'style-src': '\'self\' \'unsafe-inline\'',
//     };
//
//     const result: string = computeDirectives(policies);
//
//     expect(result).toBe('default-src \'self\';script-src \'self\' \'unsafe-inline\';style-src \'self\' \'unsafe-inline\';');
//   });
//
//   it('should compute development directive with object values using default property', () => {
//     const policies: CspPolicies<Environment> = {
//       'default-src': {
//         default: '\'self\'',
//         production: '\'self\' https://production.example.com',
//       },
//       'script-src': {
//         default: '\'self\' \'unsafe-inline\'',
//         production: '\'self\' \'unsafe-inline\' https://production.example.com',
//       },
//     };
//
//     const result: string = computeDirectives(policies);
//
//     expect(result).toBe('default-src \'self\';script-src \'self\' \'unsafe-inline\';');
//   });
//
//   it('should compute development directive with mixed string and object values', () => {
//     const policies: CspPolicies<Environment> = {
//       'default-src': '\'self\'',
//       'script-src': {
//         default: '\'self\' \'unsafe-inline\'',
//         production: '\'self\' \'unsafe-inline\' https://production.example.com',
//       },
//     };
//
//     const result: string = computeDirectives(policies);
//
//     expect(result).toBe('default-src \'self\';script-src \'self\' \'unsafe-inline\';');
//   });
//
//   it('should return empty string for empty policies', () => {
//     const policies: CspPolicies<Environment> = {};
//
//     const result: string = computeDirectives(policies);
//
//     expect(result).toBe('');
//   });
//
//   it('should replace nonce-{RANDOM} placeholders with a generated nonce', () => {
//     const policies: CspPolicies<Environment> = {
//       'default-src': '\'self\'',
//       'script-src': '\'self\' \'unsafe-inline\' nonce-{RANDOM}',
//       'style-src': '\'self\' nonce-{RANDOM}',
//     };
//
//     const result: string = computeDirectives(policies);
//
//     // Extract the generated nonce from the result
//     const nonceMatch: RegExpMatchArray | null = result.match(/script-src 'self' 'unsafe-inline' nonce-([A-Za-z0-9+/=]+);/);
//     expect(nonceMatch).not.toBeNull();
//
//     if (nonceMatch) {
//       const nonce: string = nonceMatch[1];
//
//       // Verify the nonce is used consistently across directives
//       expect(result).toContain(`script-src 'self' 'unsafe-inline' nonce-${nonce};`);
//       expect(result).toContain(`style-src 'self' nonce-${nonce};`);
//     }
//   });
//
//   it('should replace nonce-{RANDOM} placeholders in object values', () => {
//     const policies: CspPolicies<Environment> = {
//       'default-src': '\'self\'',
//       'script-src': {
//         default: '\'self\' \'unsafe-inline\' nonce-{RANDOM}',
//         production: '\'self\' \'unsafe-inline\' https://production.example.com',
//       },
//     };
//
//     const result: string = computeDirectives(policies);
//
//     // Verify the result contains a nonce
//     expect(result).toMatch(/script-src 'self' 'unsafe-inline' nonce-[A-Za-z0-9+/=]+;/);
//   });
// });
